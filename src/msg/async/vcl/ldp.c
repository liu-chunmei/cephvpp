...
ssize_t
vcl_read (int fd, void *buf, size_t nbytes)
{
  ssize_t size;
  const char *func_str;
  u32 sid = ldp_sid_from_fd (fd);
...
ssize_t
vcl_readv (int fd, const struct iovec * iov, int iovcnt)
{
  const char *func_str;
  ssize_t size = 0;
  u32 sid = ldp_sid_from_fd (fd);
...
ssize_t
vcl_write (int fd, const void *buf, size_t nbytes)
{
  const char *func_str;
  ssize_t size = 0;
  u32 sid = ldp_sid_from_fd (fd);
...
ssize_t
vcl_writev (int fd, const struct iovec * iov, int iovcnt)
{
  const char *func_str;
...
int
vcl_ioctl (int fd, unsigned long int cmd, ...)
{
  const char *func_str;
  int rv;
  va_list ap;
...
int
vcl_select (int nfds, fd_set * __restrict readfds,
	fd_set * __restrict writefds,
...
#ifdef __USE_XOPEN2K
int
vcl_pselect (int nfds, fd_set * __restrict readfds,
	 fd_set * __restrict writefds,
	 fd_set * __restrict exceptfds,
	 const struct timespec *__restrict timeout,
	 const __sigset_t * __restrict sigmask)
...
int
vcl_socket (int domain, int type, int protocol)
{
  const char *func_str;
  int rv;
 ...
 * */
int
vcl_socketpair (int domain, int type, int protocol, int fds[2])
{
  const char *func_str;
  int rv;
  int sock_type = type & ~(SOCK_CLOEXEC | SOCK_NONBLOCK);
...
int
vcl_bind (int fd, __CONST_SOCKADDR_ARG addr, socklen_t len)
{
  int rv;
  const char *func_str;
  u32 sid = ldp_sid_from_fd (fd);
...
int
vcl_getsockname (int fd, __SOCKADDR_ARG addr, socklen_t * __restrict len)
{
  int rv;
  const char *func_str;
  u32 sid = ldp_sid_from_fd (fd);
...
int
vcl_connect (int fd, __CONST_SOCKADDR_ARG addr, socklen_t len)
{
  int rv;
  const char *func_str = __func__;
  u32 sid = ldp_sid_from_fd (fd);
...
int
vcl_getpeername (int fd, __SOCKADDR_ARG addr, socklen_t * __restrict len)
{
  int rv;
  const char *func_str;
  u32 sid = ldp_sid_from_fd (fd);
...
ssize_t
vcl_send (int fd, const void *buf, size_t n, int flags)
{
  ssize_t size;
  const char *func_str;
  u32 sid = ldp_sid_from_fd (fd);
...
ssize_t
vcl_sendfile (int out_fd, int in_fd, off_t * offset, size_t len)
{
  ssize_t size = 0;
  const char *func_str;
...
ssize_t
vcl_sendfile64 (int out_fd, int in_fd, off_t * offset, size_t len)
{
  return vcl_sendfile (out_fd, in_fd, offset, len);
}

ssize_t
vcl_recv (int fd, void *buf, size_t n, int flags)
{
  ssize_t size;
  const char *func_str;
  u32 sid = ldp_sid_from_fd (fd);
...
ssize_t
vcl_sendto (int fd, const void *buf, size_t n, int flags,
...
ssize_t
vcl_recvfrom (int fd, void *__restrict buf, size_t n, int flags,
...
ssize_t
vcl_sendmsg (int fd, const struct msghdr * message, int flags)
{
  ssize_t size = 0;
  size = vcl_writev(fd, message->msg_iov,message->msg_iovlen);
  return size; 
/*
  ssize_t size;
  const char *func_str;
...
#ifdef USE_GNU
int
vcl_sendmmsg (int fd, struct mmsghdr *vmessages, unsigned int vlen, int flags)
{
  ssize_t size;
  const char *func_str;
  u32 sid = ldp_sid_from_fd (fd);
...
ssize_t
vcl_recvmsg (int fd, struct msghdr * message, int flags)
{
  ssize_t size;
  const char *func_str;
  u32 sid = ldp_sid_from_fd (fd);
...
#ifdef USE_GNU
int
vcl_recvmmsg (int fd, struct mmsghdr *vmessages,
...
int
vcl_getsockopt (int fd, int level, int optname,
...
int
vcl_setsockopt (int fd, int level, int optname,
...
int
vcl_accept4 (int fd, __SOCKADDR_ARG addr, socklen_t * __restrict addr_len,
	 int flags)
{
...
int
vcl_accept (int fd, __SOCKADDR_ARG addr, socklen_t * __restrict addr_len)
{
  return ldp_accept4 (fd, addr, addr_len, 0);
}

int
vcl_shutdown (int fd, int how)
{
  int rv;
  const char *func_str;
  u32 sid = ldp_sid_from_fd (fd);
...
int
vcl_epoll_create (int size)
{
  return epoll_create1 (0);
}

int
vcl_epoll_ctl (int epfd, int op, int fd, struct epoll_event *event)
{
  int rv;
  const char *func_str;
...
int
vcl_epoll_pwait (int epfd, struct epoll_event *events,
	     int maxevents, int timeout, const sigset_t * sigmask)
{
...
int
vcl_epoll_wait (int epfd, struct epoll_event *events, int maxevents, int timeout)
{
  return ldp_epoll_pwait (epfd, events, maxevents, timeout, NULL);
}

int
vcl_poll (struct pollfd *fds, nfds_t nfds, int timeout)
{
  const char *func_str = __func__;
...
#ifdef USE_GNU
int
vcl_ppoll (struct pollfd *fds, nfds_t nfds,
...
void
vcl_ldp_constructor (void)
{
  //swrap_constructor ();
  if (ldp_init () != 0)
    fprintf (stderr, "\nLDP<%d>: ERROR: vcl_ldp_constructor: failed!\n",
	     getpid ());
  else
    clib_warning ("LDP<%d>: VCL LDP constructor: done!\n", getpid ());
}
...
vcl_ldp_destructor (void)
{
   ...
   */
  printf ("%s:%d: LDP<%d>: VCL LDP destructor: done!\n",
	  __func__, __LINE__, getpid ());
}
